...
const ForumCard: FC<ForumCardProps> = ({ title, date, onClick }) => {
  return (
    <div
      onClick={onClick}
      className="bg-white flex justify-between items-start mb-4 rounded shadow-sm cursor-pointer hover:bg-gray-100 transition h-[100px]"
    >
      ...
      <span className="text-sm text-gray-500 p-10">{date}</span>
    </div>
  )
}
